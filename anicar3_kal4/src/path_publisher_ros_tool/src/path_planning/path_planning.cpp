...
void PathPlanning::setTaskCallback(std_msgs::Int8 msg){
   ...
   // if (!activate_) { return; }
    ...
    for (size_t i = 0; i < map_.info.height; i++)
        for (size_t j = 0; j < map_.info.width; j++) {
            ...
            if(grid_value < 51){
	    z = interface_.no_vision_part_factor;}
	    obstacle_points.emplace_back(x, y, z);
        }
    ...
    while (lat <= interface_.search_width / 2) {
        ...
        for (auto& point : obstacle_points) {
            potential += getPotential((search_point - point).norm()) * point[2];
        }
