 ...
 * @par
 *
 * write down pointclouds given as vector<Eigen::Vector3d> and geometries into plys
 */
class PlyWriter {
...
public: /* public methods */
    ...
    PlyWriter(std::string NameOfFile, bool Centered = false) : NameOfFile(NameOfFile), Centered(Centered) {
        reset();
    }
    ...
    void reset() {
        ...
        // add origin
        Body << 0.0 << " " << 0.0 << " " << 0.0 << " " << 150 << " " << 150 << " " << 150 << "\n";
        Body << 1.0 << " " << 0.0 << " " << 0.0 << " " << 150 << " " << 0 << " " << 0 << "\n";
        Body << 0.0 << " " << 1.0 << " " << 0.0 << " " << 0 << " " << 150 << " " << 0 << "\n";
        Body << 0.0 << " " << 0.0 << " " << 1.0 << " " << 0 << " " << 0 << " " << 150 << "\n";
    }
    ...
    void add_point_to_body(const Eigen::Matrix<T, 3, 1>& P, std::tuple<uint, uint, uint> RGB) {
        ...
        Body << P(0) << " " << P(1) << " " << P(2) << " " << std::get<0>(RGB) << " " << std::get<1>(RGB) << " "
             << std::get<2>(RGB) << "\n";
    }
    ...
    void add_point(const Eigen::Matrix<T, 3, 1>& P, std::tuple<int, int, int> RGB) {
        ...
        bool c = std::is_convertible<T, double>::value;
        assert(c);
        std::tuple<uint, uint, uint> UIntRGB =
            std::make_tuple(uint(std::get<0>(RGB)), uint(std::get<1>(RGB)), uint(std::get<2>(RGB)));
    ...
    /**
     * @brief add a square in the given plane with Area Edgelength^2 around PointNearMiddle(does not have to be part of
     * the plane)
     * @return  void
     * @par ...
     */
