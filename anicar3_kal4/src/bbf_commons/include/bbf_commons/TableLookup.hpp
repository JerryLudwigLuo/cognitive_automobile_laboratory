...
{
public:
  TableLookup(double _minVal, double _maxVal, const std::vector<double>& _funVals) :
  ...
  double getVal(double val, LookupMode mode=linear)
  {
    ...
    switch(mode)
    {
      ...
      case linear:
      {
        int idxPrev = std::max((int) floor(idxRel), 0);
        int idxNext = std::min((int) idxPrev+1, (int) funVals.size() - 1);
